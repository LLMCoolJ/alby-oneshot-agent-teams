...
Clone the repo and start a fresh branch from the `START-HERE` tag to see what existed before any code was generated:
...
This gives you a clean working tree with only the pre-implementation files — no generated code.
...
This creates a new branch from the `START-HERE` tag with only the pre-implementation files (specs, skills, prompts — no generated code). Then tell Claude Code:

```
@IMPLEMENT.md
```

This triggers the full sequential build of all 15 specs. Your implementation commits will land on `my-build`.
